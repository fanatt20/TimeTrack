    ...
    public class ProcessSessionsImporter
    {
        public static void ImportFromCSV(IProcessSessionRepository repo, string pathAndName)
        {
            ...
            using (var streamReader = new StreamReader(pathAndName))
            {
                ...
                {
                    var str = streamReader.ReadLine().Split(new[] {"\",\""}, StringSplitOptions.None);
                    repo.Add(new ProcessSession(str[0].Remove(0, 1), str[1], DateTime.Parse(str[2]),
                        TimeSpan.Parse(str[3].Remove(str[3].Length - 1))));
                }
            }
        }

        public static void DeserializeFromFile(IProcessSessionRepository repo, string pathAndName)
        {
